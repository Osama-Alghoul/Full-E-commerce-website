import Breadcrumbs from "../../ui/BreadCrumb";
import { useAuthContext } from "../../../utils/AuthContext";
import Login from "../log-in/Login";
import { CheckCircleIcon, Loader } from "lucide-react";
import Button from "../../ui/Button";
import { Link } from "react-router";
import { useEffect, useState } from "react";

export default function PaymentStatus() {
  const { isLoggedIn } = useAuthContext();
  const [status, setStatus] = useState(false);
  
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  useEffect(() => {
    setTimeout(() => {
      setStatus(true);
    }, 2000);
  }, []);

  if (!isLoggedIn) {
    return <Login />;
  }

  return (
    <>
      <Breadcrumbs
        items={[
          { label: "Home", path: "/" },
          { label: "View Cart", path: "/cart" },
          { label: "CheckOut", path: "/cart/checkout" },
          { label: "Payment Status", path: "/cart/checkout/status" },
        ]}
      />
      <section
        className={`flex ${
          status ? "flex-col" : ""
        } justify-center w-full gap-5 items-center pb-48`}
      >
        {status ? (
          <>
            <div className="flex gap-5">
              <CheckCircleIcon className="size-10 text-green-400" />
              <h1 className="lg:text-5xl md:text-4xl text-3xl">Payment completed successfully</h1>
            </div>
            <Link to="/" className="pt-12">
              <Button>Back to Home</Button>
            </Link>
          </>
        ) : (
          <>
            <Loader className="animate-spin size-10" />
            <h1 className="lg:text-5xl md:text-4xl text-3xl">Your Payment in progress</h1>
          </>
        )}
      </section>
    </>
  );
}
